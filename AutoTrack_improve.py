...
class AutoTrack:
    ...
    def _train(self, frame, response=None, disp=None):
        xf = self._get_features(frame, self.pos)
        #生成目标和背景掩码
        mask_t, mask_b = self.generate_target_background_mask((self.Hc, self.Wc), 0.5)
        xt = xf * mask_t #目标
        xb = xf * mask_b #背景

        xf = np.fft.fft2(xf, axes=(0, 1))
        xb = np.fft.fft2(xb, axes=(0, 1))
        xt = np.fft.fft2(xt, axes=(0, 1))
        ...
        S_tt = np.sum(np.conj(xt) * xt, axis=2)
        S_bb = np.sum(np.conj(xb) * xb, axis=2)
        ...
        for _ in range(self.admm_iter):
            # ===== g 子问题 =====
            B = S_xx + gamma + mu - self.lamda1 * S_tt + self.lamda2 * S_bb
            term1 = yf[..., None] * xf
            term2 = mu * self.g_pre
            term3 = gamma * self.h_f
            term4 = -self.l_f

            self.g_f = (term1 + term2 + term3 + term4) / B[..., None]
    ...
    #生成目标和背景掩码,输入窗口大小，掩码尺寸比例
    def generate_target_background_mask(self, window_sz, ratio):
        H, W = window_sz
        mask_2d = np.zeros((H, W), np.float32)
        ...
        #目标区域都为1
        mask_2d[h_start:h_end, w_start:w_end] = 1.0

        mask_t = mask_2d[:, :, None]
        mask_b = 1.0 - mask_t
        return mask_t, mask_b
